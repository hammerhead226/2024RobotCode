  ...
  private void configureButtonBindings() {
    ...
    if (aPresses == 0) {
      ...
      controller
          .a()
          .onTrue(
              new SetElevatorTarget(Constants.ElevatorConstants.EXTEND_SETPOINT_INCH, elevator));

    } else if (aPresses == 1) {
      controller
          .a()
          .onTrue(
              new SetElevatorTarget(Constants.ElevatorConstants.RETRACT_SETPOINT_INCH, elevator));
      aPresses++;
    } else {
      controller
          .a()
                  ...
                  new InstantCommand(
                      () ->
                          shooter.setFlywheelRPMs(
                              Constants.ShooterConstants.LEFT_FLYWHEEL_TRAP_SPEED,
                              Constants.ShooterConstants.RIGHT_FLYWHEEL_TRAP_SPEED)),
                  null));
    }

    if (elevator.getExtenderPosition() == Constants.ElevatorConstants.EXTEND_SETPOINT_INCH) {
      controller
          .x()
          .onTrue(
              new SetElevatorTarget(Constants.ElevatorConstants.EXTEND_STOW_SETPOINT, elevator));
    } else {
      controller
          .x()
          .onTrue(
